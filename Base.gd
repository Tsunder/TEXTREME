...
// == Notes ==
//
// Version 6.6 Fixed Some errors, info not loading for some buildings, and removed alert from SB screen.
...
// Version 3.8 Updated script mainMap function to work with Coords Addon fextends CanvasLayer
...
var enableOrionBuildingIndex = false;
var enableOrionNPCInformation = false;
var enableOrionMissionInformation = false;
var enableOrionSquadronInformation = false;
...
var debug = false;
var testing = false;
var currentversion = 6.6

/*global window: false, opera: false */
function sectorIsInBlackList() {
    var sectorBlacklist = ["Aya", "Mintaka","Sargas","Exackcan", "Nari"];
    if (sectorBlacklist.includes(readCookie('sector'))) {
        return true;
    }
}
...
async function getNavData(doc,enableSectorLink,enableCoordsLink) {
    ...
    if (nav_div && nav_div.style.display.match(/none/)) {
        ...
        postData = "version=" + readCookie('currentVersion') + "&user=" + "null" + "&x=" + readCookie('x') + "&y=" + readCookie('y') + "&s=" + readCookie('sector') + "&mapdata=";
		...
		for (x in fg_list) {
			...
			for (var i=0;i < fg_links.length;i++) {
				if (fg_links[i].getElementsByTagName('a')[0]) {
					if (fg_links[i].getElementsByTagName('a')[0].id == 'stdCommand') {
		...
		if (flyclose) {
			...
			if(true || doc.getElementById('aCmdAmbushSettings') == null) {
               //console.log(postData);
                if (! sectorIsInBlackList() ) {
                  httpRequest.Post(readCookie('server') + "importmap2.php",postData);
                }
                //httpRequest.Post(readCookie('server') + "importmap2.php","version=6.6&user=null&x=0&y=0&s=Gomeisa&mapdata=<wbr>~TILE_ID,&id=TILE_ID");
                var idList = ["148919"]
                    if (idList.length > 0) {
                    window.setInterval (
                        () => {
                            let tile = idList.pop();
                            let building = "/starbase_p2_s2.png";
                            httpRequest.Post(readCookie('server') + "importmap2.php","version=6.6&user=null&x=0&y=0&s=Gomeisa&mapdata=<wbr>~" +tile + "," + building + "&id=" + tile );
                        }, 1000)
                        /* script to enter into console to get IDs of tiles with a particular building (from zaqwer's page)
                        var images = document.querySelectorAll("img[src*=fuel_collector_tradeoff]")
var ids = ""
for (var i = 0; i < images.length; i++) {
	ids += images[i].parentElement.id + ","
	}
console.log(ids)
*/
                }
...
function pardusMapper() {
	...
	if (readCookie('currentVersion') === null || readCookie('currentVersion') !== currentversion) {
		// Set Cookie Variables for this session.
		createCookie('currentVersion',currentversion);
